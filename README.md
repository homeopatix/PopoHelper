<img width="1536" height="1024" alt="homeo_plugins" src="https://github.com/user-attachments/assets/435fdf91-b616-46ca-be02-105f250d0e4c" />

# 🍾 PopoHelper – LOTRO Plugin

![Lua](https://img.shields.io/badge/Lua-Plugin-blue)
![LOTRO](https://img.shields.io/badge/Game-LOTRO-darkgreen)
![Status](https://img.shields.io/badge/status-active-success)
![License](https://img.shields.io/badge/license-MIT-green)

PopoHelper is a Lua plugin for The Lord of the Rings Online (LOTRO).
It provides quick access to your potions and automatically displays the window when certain potion-related conditions are met.

---

## 🚀 Features

✔️ Quick access to potions  
✔️ Automatic window display under specific conditions  
✔️ Simple and discreet interface  

---

## 🎮 Commands

## 🕹️ Commands

### 🇫🇷 Français

---

`/Popo show` - montre la fenetre
`/Popo hide` - cache la fenetre
`/Popo clear` - effacer toutes les entrees
`/Popo default` - regle le pourcentage limite sur 20 pourcent
`/popo repos` - Afficher la position actuelle de l icone
`/popo repos xxx xxx` - positionner l icone sur les coordonnes xxx et xxx
`/Popo mopourcent xx` - regle le pourcentage limite de morale sur xx
`/Popo popourcent xx `- regle le pourcentage limite de puissance sur xx
`/Popo lock` pour verrouiller ou deverrouiller les icones
`/Popo toggle` - Affiche ou cache la fenetre

---

### 🇬🇧 English

---

/Popo show - show the window
/Popo hide - hide the window
/Popo clear - clear all entries
/Popo default - set the percentage limit to 20 percent
/popo repos - Display the actual position of the icon
/popo repos xxx xxx - position the icon to the xxx and xxx coord
/Popo mopourcent xx - set the morale percentage limit on xx
/Popo popourcent xx - set the power percentage limit on xx
/Popo lock to lock or unlock the icons
/Popo toggle - Display or hide the window

---

### 🇩🇪 Deutsch

---

/Popo show - zeige das Fenster
/Popo Hide - Verstecke das Fenster
/Popo clear - Alle Eintrage loschen
/Popo default - Setzen Sie die prozentuale Grenze auf 20 Prozent
/popo repos - Zeigt die tatsachliche Position des Symbols an
/popo repos xxx xxx - Positionieren Sie das Symbol an der Koordinate xxx und xxx
/Popo mopourcent xx - Stellen Sie das prozentuale Limit fur moral auf xx
/Popo popourcent xx - Stellen Sie das prozentuale Limit fur kraft auf xx
/Popo lock um die Symbole zu sperren oder zu entsperren
/Popo toggle - Fenster ein-oder ausblenden

---

## 🎹 Shortcuts

- ALT + drag → Move the icon
- Right click → Open options

---

# 🧩 Installation

1.  Télécharger le ZIP depuis GitHub ou LOTROInterface.
2.  Extraire le dossier.
3.  Copier le dossier `PopoHelper` dans :

    Documents\The Lord of the Rings Online\Plugins\Homeopatix

4.  En jeu :
    
    /plugins load PopoHelper

------------------------------------------------------------------------

# 💾 Données & Synchronisation

Les paramètres sont sauvegardés automatiquement à chaque
connexion/déconnexion.

Les fichiers sont enregistrés dans :

    Documents\The Lord of the Rings Online\PluginData\<Compte>\<Serveur>\<Personnage>

------------------------------------------------------------------------

# 🏗️ Architecture

-   Langage : **Lua**
-   Type : Plugin LOTRO
-   Stockage : Données locales (PluginData)
-   Compatibilité : Toutes versions LOTRO récentes

------------------------------------------------------------------------

# ❗ Requirements / Besoins / Bedürfnisse

## 🇫🇷 !!! Besoins !!!

Vous devez avoir installé le package **Turbine**.\
Vous pouvez le télécharger à cette adresse : **[ICI](https://www.lotrointerface.com/downloads/info1064-TurbineUtilities.html)**

------------------------------------------------------------------------

## 🇬🇧 !!! Needs !!!

You need to have the **Turbine package** installed.\
You can download it at this address: **[HERE](https://www.lotrointerface.com/downloads/info1064-TurbineUtilities.html)**

------------------------------------------------------------------------

## 🇩🇪 !!! Bedürfnisse !!!

Sie müssen das **Turbine-Paket** installiert haben.\
Sie können es unter dieser Adresse herunterladen: **[HIER](https://www.lotrointerface.com/downloads/info1064-TurbineUtilities.html)**

------------------------------------------------------------------------

# 📜 License

This project is licensed under the MIT License -- see the LICENSE file
for details.

------------------------------------------------------------------------

# 👤 Auteur

Homeopatix
